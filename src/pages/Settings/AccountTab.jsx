...
import { selectCurrentUser, updateAPI } from "~/redux/user/userSlice";
import {
...
function AccountTab() {
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    // Gọi API...
    toast
      .promise(dispatch(updateAPI({ displayName })), { pending: "Updating..." })
      .then((res) => {
        if (!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    // console.log("reqData: ", reqData); => Không log được
    ...
    // Gọi API...
    toast
      .promise(dispatch(updateAPI(reqData)), { pending: "Updating..." })
      .then((res) => {
        if (!res.error) {
          toast.success("User updated successfully!");
        }
        // Lưu ý: dù có lỗi hay thành công thì cũng phải clear giá trị của file input nếu không thì không thể chọn cùng 1 file liên tiếp đươc
        // Lấy giá trị là e.target?.files[0] nhưng clear thì phải là e.target.value
        e.target.value = "";
      });
  };
